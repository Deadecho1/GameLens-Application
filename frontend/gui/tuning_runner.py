...
class TuningRunner(QObject):
    ...
    def start_tuning(
        ...
        for model_id in enabled_model_ids:
            ...
            # Gather all user marks for this model across all videos
            all_model_marks: list[dict] = []
            for video in videos:
                for mark in video.get("marks", []):
                    if mark.get("modelId") == model_id:
            ...
            # Determine none clip target count
            ...
            for video in videos:
                ...
                video_marks = [
                    m for m in video.get("marks", []) if m.get("modelId") == model_id
                ]
                ...
                # Build per-video none count proportional to mark count
                video_none_count = 0
                if none_target > 0 and duration > 0:
                    total_marks = len(all_model_marks)
                    video_none_count = max(
                        1,
                        round(none_target * len(video_marks) / total_marks),
                    )

                none_events = _compute_none_events(
                    marked_times, duration, video_none_count
                )

                events = [
                    {"time": m["time"], "label": m["categoryId"]}
                    for m in video_marks
                ] + none_events
        ...
        if not self._queue:
            self._status = "idle"
            self.pipeline_finished_helper(False, "Nothing to run — no valid marks found.")
            return
